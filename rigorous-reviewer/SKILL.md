...
3. `references/failure_mode_playbook.md` - final red-line audit and rewrite
   rules.
4. `references/agent_behavior_core.md` - scope discipline, assumption ledger,
   surgical output rules, anti-slop filter, and verification reporting.
5. `references/issue_traceability_and_scope_discipline.md` - claim/figure/method/source
   traceability requirements for every Critical and Major issue.
6. `references/final_verification_contract.md` - final pre-return checks for
   evidence sufficiency, issue anatomy, version/license consistency, and validation.
...
1. **Reconstruct the paper.** State the authors' strongest take-home message,
   ...
   evidence threshold.
2. **Apply scope and traceability discipline.** Before generating criticism,
   record assumptions, user-provided material boundaries, and the exact claim,
   figure, method, dataset, proof step, benchmark, or source standard that each
   candidate issue would trace to. Discard generic reviewer-sounding comments
   that cannot be traced.
3. **Screen user-supplied material first.** Do not silently skip supplied
...
4. **Lock the pre-review contract.** Before final severity or recommendation,
...
5. **Identify domains.** Map the manuscript to biology, chemistry, physics,
   mathematics, medicine, computer science, and any fusion bridge claim. Use
   `six_domain_review_standards.md`.
6. **Build an evidence dossier.** Use manuscript-internal evidence plus external
...
7. **Calibrate.** Use `calibration_protocol.md` to compare against gold,
   near-gold, negative, and boundary anchors. Estimate FNR/FPR/balanced accuracy
   only when labeled anchors make that defensible.
8. **Assign claim maturity.** Use Level 0-3 maturity for each central claim so
...
9. **Map claim-to-readout dependencies when relevant.** If a claim depends on
...
10. **Use companion skills if available.** Follow
...
11. **Use MCP capabilities if available.** Follow
   ...
   synthesis.
12. **Respect phase boundaries.** Intake, evidence audit, reviewer passes, and
...
13. **Audit figure-claim contracts when relevant.** For each central figure or
...
14. **Separate rhetoric from evidence.** Do not let polished writing, journal
...
15. **Run reviewer-panel passes.** Generate separate EIC, methods/statistics,
   domain, interdisciplinary, Devil's Advocate, and editorial synthesis findings
   before final writing.
16. **Score and rank issues.** Use `rubric.json`; rank by claim dependency and
   severity, not rhetorical intensity.
17. **Write professional issue blocks, not short comments.** Every Critical,
   ...
   `解决：`, and `决定性 readout：`.
18. **Convert to action.** Separate essential revisions from important but
   non-decisive improvements and wording/reporting fixes.
19. **Generate search hints.** Every central evidence gap needs targeted source
    routes, search strings, and decision-changing evidence.
20. **Recommend.** Choose Accept, Minor Revision, Major Revision, or Reject from
    unresolved claim dependency, novelty, severity distribution, loophole burden,
    and revision feasibility.
21. **Format as Markdown.** Default to a Markdown report in chat. Save a `.md`
...
22. **Audit and rewrite.** Apply `failure_mode_playbook.md`; rewrite any section
    that fails a red line before returning the report.
23. **Validate saved reports.** If a `.md` report file is created, run
    `scripts/validate_review_report.py <report.md>` and fix any failures before
    handing it back.
24. **Validate structured artifacts.** If a JSON review artifact is created, run
