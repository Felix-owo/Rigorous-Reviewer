...
- Material basis: synthetic abstract, figures, methods summary, and benchmark description.
- Central claim dependency: C1, pathway X is a causal driver of phenotype Y.
- Decisive evidence threshold: perturbing pathway X changes phenotype Y in the predicted direction and rescue restores the phenotype.
- Failure condition: absence of perturbation/rescue or equivalent causal design keeps the causal mechanism claim at Major Revision or forces claim narrowing.
- Prohibited post-hoc shift: prediction accuracy cannot replace causal perturbation evidence.

## 3) Claim Maturity Gate

| Claim | Current maturity | Evidence basis | Blocking loophole | Upgrade readout | Required narrowing |
| --- | ---: | --- | --- | --- | --- |
| C1: pathway X is a causal driver of phenotype Y | 2 | Associative model and enrichment evidence are present, but perturbation/rescue is absent. | Batch, cell composition, disease severity, or stress-response confounding can explain the association. | Perturbation changes phenotype Y and rescue restores it under donor-level replication. | If causal evidence is absent, narrow to candidate prioritization associated with phenotype Y. |
